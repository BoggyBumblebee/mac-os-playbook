...
By default the VM flow installs missing guest prerequisites, pre-seeds the configured dotfiles checkout for check-mode validation, runs `ansible-galaxy`, a syntax check, and an Ansible check-mode pass with `--skip-tags mas,post`. Add `--real --idempotence` after the check-mode output looks safe:
